...
const companionPadStyle: CSSProperties = { padding: "18px" };
const headerGridStyle: CSSProperties = { display: "grid", gridTemplateColumns: "86px minmax(0, 1fr) auto", gap: "14px", alignItems: "center" };
const mascotBadgeStyle: CSSProperties = { alignItems: "center", background: "linear-gradient(135deg, rgba(234, 179, 8, 0.16), rgba(239, 68, 68, 0.12))", border: "1px solid rgba(250, 204, 21, 0.28)", borderRadius: "22px", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.08)", display: "flex", height: "86px", justifyContent: "center", overflow: "hidden", position: "relative" };
const mascotImageStyle: CSSProperties = { display: "block", height: "78px", objectFit: "contain", width: "78px" };
const confidencePillBaseStyle: CSSProperties = { borderRadius: "999px", display: "inline-flex", flexDirection: "column", gap: "1px", minWidth: "112px", padding: "9px 12px", textAlign: "right" };
const categoryGridStyle: CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(128px, 1fr))", gap: "9px", marginTop: "16px" };
const categoryButtonBaseStyle: CSSProperties = { border: "1px solid rgba(148, 163, 184, 0.2)", borderRadius: "16px", color: "#fff", cursor: "pointer", minHeight: "82px", padding: "11px", textAlign: "left" };
...
const insightPanelStyle: CSSProperties = { borderTop: "1px solid rgba(255,255,255,0.08)", display: "grid", gap: "12px", gridTemplateColumns: "minmax(0, 1fr) minmax(220px, 0.7fr)", marginTop: "16px", paddingTop: "16px" };
...
const guidanceItemStyle: CSSProperties = { alignItems: "flex-start", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", display: "grid", gap: "10px", gridTemplateColumns: "28px minmax(0, 1fr)", padding: "10px" };
const lensGridStyle: CSSProperties = { display: "grid", gap: "8px", gridTemplateColumns: "repeat(auto-fit, minmax(136px, 1fr))", marginTop: "12px" };
const promptSectionStyle: CSSProperties = { borderTop: "1px solid rgba(255,255,255,0.08)", marginTop: "16px", paddingTop: "16px" };
const promptGridStyle: CSSProperties = { display: "grid", gap: "10px", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))" };
const promptButtonStyle: CSSProperties = { borderRadius: "16px", color: "#fff", cursor: "pointer", display: "flex", flexDirection: "column", gap: "7px", minHeight: "108px", padding: "12px", textAlign: "left" };
const askAiButtonStyle: CSSProperties = { alignItems: "center", background: "linear-gradient(135deg, #2563eb, #1d4ed8)", border: "1px solid rgba(147, 197, 253, 0.38)", borderRadius: "14px", color: "#fff", display: "inline-flex", gap: "10px", fontSize: "0.8rem", fontWeight: 950, padding: "10px 12px" };
...
function buildQaLenses(draft: string, extraContext = ""): QaLens[] {
  const signals = getSignals(draft, extraContext);
  const lenses: QaLens[] = [];
  if (signals.hasRoles) lenses.push({ id: "permissions", label: "Permissions lens", detail: "Define allowed roles, blocked roles, and safe failure messaging.", tone: "yellow" });
  if (signals.hasAsync) lenses.push({ id: "async", label: "Async/state lens", detail: "Cover loading, retry, stale state, duplicate actions, and partial success.", tone: "red" });
  if (signals.hasAi) lenses.push({ id: "ai", label: "AI review lens", detail: "Keep generated output reviewable, editable, and grounded in source context.", tone: "blue" });
  if (signals.hasBilling) lenses.push({ id: "billing", label: "Billing/credits lens", detail: "Protect against duplicate charges, failed spends, refunds, and unclear pricing.", tone: "red" });
  if (signals.hasData) lenses.push({ id: "data", label: "Data persistence lens", detail: "Confirm save, restore, ownership, deletion, and source-of-truth behavior.", tone: "green" });
  if (signals.hasIntegration) lenses.push({ id: "integration", label: "Integration lens", detail: "Plan auth, API errors, field mapping, permission failures, and preview-first sync.", tone: "yellow" });
  if (signals.hasUi) lenses.push({ id: "ui", label: "UX clarity lens", detail: "Check empty, loading, error, success, mobile, and keyboard states.", tone: "blue" });
  if (!lenses.length && draft.trim()) lenses.push({ id: "general", label: "General QA lens", detail: "Add failure states, ownership, acceptance criteria, and regression areas.", tone: "yellow" });
...
function buildGuidanceFeed(args: { draft: string; extraContext: string; projectName: string; productType: string; readinessScore: number; confidence: ContextConfidence; lenses: QaLens[] }): GuidanceItem[] {
  ...
  if (!args.draft.trim()) items.push({ icon: "🧱", title: "Start rough", detail: "Give me the messy idea. I’ll help turn it into scope, criteria, risks, and Jira work.", tone: "info" });
  if (args.confidence === "minimal") items.push({ icon: "🟡", title: "Minimal context mode", detail: "I can still help, but project name, product type, or constraints will make the guidance sharper.", tone: "watch" });
  if (signals.hasAi) items.push({ icon: "🤖", title: "AI workflow detected", detail: "Make generated output reviewable before users save, sync, or create Jira work.", tone: "info" });
  if (signals.hasIntegration) items.push({ icon: "🔌", title: "Integration surface detected", detail: "Plan for auth failures, permission mismatches, field mapping, timeouts, and retry states.", tone: "watch" });
  if (signals.hasBilling) items.push({ icon: "💳", title: "Credit/billing risk detected", detail: "Protect duplicate clicks, failed charges, already-spent states, and confusing pricing copy.", tone: "risk" });
  if (missing[0]) items.push({ icon: "🔎", title: `${missing[0].label} is the next gap`, detail: missing[0].detail, tone: "watch" });
  if (!signals.hasRisk && args.draft.trim()) items.push({ icon: "⚠️", title: "No risk lens yet", detail: "Add failure modes, permissions, stale state, bad input, or regression areas before Jira creation.", tone: "risk" });
  if (signals.hasAcceptance && signals.hasScope && signals.hasRisk) items.push({ icon: "✅", title: "This is getting buildable", detail: "Scope, acceptance, and risk signals are present. Next step: make Jira work previewable.", tone: "good" });
  if (args.projectName && items.length < 4) items.push({ icon: "🧠", title: `Using ${args.projectName}`, detail: "Project context is available for this builder session. Avoid polling; refresh context intentionally when needed.", tone: "info" });
  if (!items.length) items.push({ icon: "✅", title: "Ready for the next pass", detail: "The idea has enough structure for QAt to help refine toward a feature brief.", tone: "good" });
...
function buildBuilderActions(draft: string, extraContext = ""): BuilderAction[] {
  ...
  if (!signals.hasRisk || lenses.length) actions.push({ title: "Add QA risks", detail: "Turn detected lenses into test targets.", tone: "red", prompt: `QA risks to cover:\n${(lenses.length ? lenses : [{ label: "General QA lens", detail: "Failure states, permissions, stale state, bad input, and regression areas." }]).map((lens) => `- ${lens.label}: ${lens.detail}`).join("\n")}` });
...
function getBriefingText(args: { readinessScore: number; confidence: ContextConfidence; qaLenses: QaLens[]; missingCallouts: MissingCallout[] }) {
  ...
  const lensNames = args.qaLenses.map((lens) => lens.label.replace(" lens", "").toLowerCase());
  if (args.readinessScore >= 80) return "This is nearly Jira-ready. Use the final pass to verify acceptance criteria, risk coverage, and child task shape.";
  if (lensNames.length) return `QAt is watching ${lensNames.slice(0, 2).join(" and ")} risk. Resolve ${missing[0] || "the next open gap"} before creating work.`;
  return `QAt needs ${missing[0] || "more detail"} next. Add one concrete detail and the builder will tighten the next recommendation.`;
}
      ...
      <div style={companionPadStyle}>
        <div className="feature-companion-header" style={headerGridStyle}>
          ...
          <div>
            ...
            <p style={{ color: mascotState === "ready" ? "#86efac" : mascotState === "concern" ? "#fde68a" : mascotState === "thinking" ? "#bfdbfe" : "rgba(229,231,235,0.58)", fontSize: "0.72rem", fontWeight: 900, margin: "8px 0 0" }}>QAt status: {mascotState === "ready" ? "ready to shape Jira work" : mascotState === "concern" ? "reviewing risk lenses" : mascotState === "thinking" ? "thinking through prompts" : "waiting for feature detail"}</p>
          </div>
          <div style={{ ...confidencePillBaseStyle, background: confidenceCopy.background, border: confidenceCopy.border }}><strong style={{ color: confidenceCopy.color, fontSize: "0.78rem" }}>{confidenceCopy.label}</strong><span style={{ color: "rgba(229, 231, 235, 0.64)", fontSize: "0.68rem", lineHeight: 1.25 }}>{confidenceCopy.detail}</span></div>
        </div>

        <div style={{ ...guidanceCardStyle, marginTop: "14px", borderColor: "rgba(96, 165, 250, 0.24)", background: "linear-gradient(135deg, rgba(30,64,175,0.16), rgba(0,0,0,0.22))" }}>
          <p className="report-kicker" style={{ marginBottom: "6px" }}>QAt briefing</p>
          <p style={{ color: "rgba(229,231,235,0.78)", fontSize: "0.82rem", lineHeight: 1.45, margin: 0 }}>{briefingText}</p>
        </div>

        <div className="feature-companion-meter" aria-label={`Readiness score ${readinessScore}%`} style={{ marginTop: "16px" }}>
          <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: "12px", marginBottom: "7px" }}><span style={{ color: "rgba(229, 231, 235, 0.74)", fontSize: "0.76rem", fontWeight: 900, letterSpacing: "0.12em", textTransform: "uppercase" }}>Feature readiness</span><strong style={{ color: readinessScore >= 80 ? "#86efac" : readinessScore >= 50 ? "#fde68a" : "#fecaca" }}>{readinessScore}%</strong></div>
          <div style={{ background: "rgba(15, 23, 42, 0.9)", border: "1px solid rgba(148, 163, 184, 0.18)", borderRadius: "999px", height: "10px", overflow: "hidden" }}><span style={{ background: readinessScore >= 80 ? "linear-gradient(90deg, #22c55e, #86efac)" : readinessScore >= 50 ? "linear-gradient(90deg, #f59e0b, #fde68a)" : "linear-gradient(90deg, #ef4444, #fca5a5)", borderRadius: "999px", display: "block", height: "100%", width: `${readinessScore}%` }} /></div>
        </div>

        <div className="feature-companion-tabs" role="tablist" aria-label="Prompt categories" style={categoryGridStyle}>{(Object.keys(CATEGORY_LABELS) as PromptCategory[]).map((category) => { const isActive = activeCategory === category; return <button aria-selected={isActive} className={isActive ? "active" : ""} key={category} onClick={() => { setActiveCategory(category); setAiPrompts([]); setAiNextQuestion(""); setAiError(""); }} role="tab" style={{ ...categoryButtonBaseStyle, ...(isActive ? activeCategoryButtonStyle : inactiveCategoryButtonStyle) }} type="button"><strong style={{ display: "block", fontSize: "0.88rem", marginBottom: "6px" }}>{CATEGORY_LABELS[category]}</strong><span style={{ color: "rgba(229, 231, 235, 0.68)", display: "block", fontSize: "0.72rem", lineHeight: 1.35 }}>{CATEGORY_DESCRIPTIONS[category]}</span></button>; })}</div>

        {builderActions.length ? <section aria-label="Recommended builder actions" style={{ ...guidanceCardStyle, marginTop: "14px" }}><div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: "10px" }}><h4 style={{ color: "#fff", fontSize: "0.92rem", margin: 0 }}>Recommended builder actions</h4><span style={{ color: "rgba(229, 231, 235, 0.52)", fontSize: "0.72rem", fontWeight: 850 }}>Click to append</span></div><div style={promptGridStyle}>{builderActions.map((action) => <button key={action.title} onClick={() => onAppendPrompt(action.prompt)} style={{ ...getPromptStyle(action.tone), minHeight: "86px" }} type="button"><strong style={{ color: "#fff", fontSize: "0.85rem" }}>{action.title}</strong><span style={{ color: "rgba(229,231,235,0.68)", fontSize: "0.74rem", lineHeight: 1.35 }}>{action.detail}</span></button>)}</div></section> : null}

        {qaLenses.length ? <section aria-label="Active QA lenses" style={{ ...guidanceCardStyle, marginTop: "14px" }}><div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: "10px" }}><h4 style={{ color: "#fff", fontSize: "0.92rem", margin: 0 }}>Active QA lenses</h4><span style={{ color: "rgba(229, 231, 235, 0.52)", fontSize: "0.72rem", fontWeight: 850 }}>Detected from draft</span></div><div style={lensGridStyle}>{qaLenses.map((lens) => <div key={lens.id} style={getLensStyle(lens.tone)}><strong style={{ color: "#fff", display: "block", fontSize: "0.78rem" }}>{lens.label}</strong><span style={{ color: "rgba(229, 231, 235, 0.66)", display: "block", fontSize: "0.69rem", lineHeight: 1.35, marginTop: "4px" }}>{lens.detail}</span></div>)}</div></section> : null}

        <div style={insightPanelStyle}>
          <section aria-label="QAt builder guidance" style={guidanceCardStyle}><div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: "10px", marginBottom: "10px" }}><h4 style={{ color: "#fff", fontSize: "0.92rem", margin: 0 }}>QAt builder notes</h4><span style={{ color: "rgba(229, 231, 235, 0.52)", fontSize: "0.72rem", fontWeight: 850 }}>Live, local</span></div><div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>{guidanceFeed.map((item) => <div key={`${item.title}-${item.detail}`} style={getGuidanceStyle(item.tone)}><span style={{ fontSize: "1.1rem", lineHeight: 1 }}>{item.icon}</span><div><strong style={{ color: "#fff", display: "block", fontSize: "0.82rem", lineHeight: 1.35 }}>{item.title}</strong><span style={{ color: "rgba(229, 231, 235, 0.68)", display: "block", fontSize: "0.74rem", lineHeight: 1.4, marginTop: "3px" }}>{item.detail}</span></div></div>)}</div></section>
          <section className="feature-companion-missing-panel" aria-label="Feature readiness callouts" style={guidanceCardStyle}><h4 style={{ color: "#fff", fontSize: "0.92rem", margin: "0 0 10px" }}>Readiness reasoning</h4><div className="feature-companion-callouts" style={{ display: "flex", flexDirection: "column", gap: "8px" }}>{missingCallouts.map((callout) => <div className={callout.isReady ? "ready" : ""} key={callout.label} style={{ border: callout.isReady ? "1px solid rgba(134, 239, 172, 0.24)" : "1px solid rgba(248, 113, 113, 0.18)", borderRadius: "13px", background: callout.isReady ? "rgba(22, 101, 52, 0.14)" : "rgba(0, 0, 0, 0.22)", padding: "9px 10px" }}><strong style={{ color: callout.isReady ? "#86efac" : "#fecaca", display: "block", fontSize: "0.78rem" }}>{callout.isReady ? "✓ " : "• "}{callout.label}</strong><span style={{ color: "rgba(229, 231, 235, 0.62)", display: "block", fontSize: "0.7rem", lineHeight: 1.35, marginTop: "3px" }}>{callout.detail}</span></div>)}</div></section>
        </div>

        <section className="feature-companion-next-question" style={{ ...guidanceCardStyle, marginTop: "12px" }}><h4 style={{ color: "#fff", fontSize: "0.92rem", margin: "0 0 8px" }}>Suggested next move</h4><button onClick={() => onAppendPrompt(nextQuestion)} style={{ border: "1px solid rgba(250, 204, 21, 0.32)", borderRadius: "14px", background: "linear-gradient(135deg, rgba(113, 63, 18, 0.46), rgba(15, 23, 42, 0.86))", color: "#fff", cursor: "pointer", fontWeight: 850, lineHeight: 1.4, padding: "11px 12px", textAlign: "left", width: "100%" }} type="button">{nextQuestion}</button></section>

        <section className="feature-companion-prompt-section" style={promptSectionStyle}><div className="feature-companion-section-header" style={{ alignItems: "center", display: "flex", gap: "12px", justifyContent: "space-between", marginBottom: "12px" }}><div><h4 style={{ color: "#fff", fontSize: "0.94rem", margin: 0 }}>{CATEGORY_LABELS[activeCategory]} prompts</h4><p style={{ color: "rgba(229, 231, 235, 0.6)", fontSize: "0.75rem", lineHeight: 1.4, margin: "4px 0 0" }}>Click a card to add it to the draft. Local prompts are free; AI prompts cost credits only when requested.</p></div><button disabled={!canAskAi} onClick={requestAiPrompts} style={askAiButtonStyle} type="button"><span>{isLoadingAiPrompts ? "Asking AI..." : "Ask AI"}</span><strong>1 credit</strong></button></div>
          ...
          <div className="feature-builder-prompt-list feature-companion-prompt-list" style={promptGridStyle}>{[...aiPrompts, ...localPrompts].slice(0, 8).map((item, index) => <button className="feature-builder-prompt" key={`${item.title}-${item.prompt}-${index}`} onClick={() => onAppendPrompt(item.prompt)} style={getPromptStyle(item.tone, index < aiPrompts.length)} type="button"><strong style={{ color: "#fff", fontSize: "0.88rem", lineHeight: 1.25 }}>{item.title}</strong><span style={{ color: "rgba(229, 231, 235, 0.72)", fontSize: "0.78rem", lineHeight: 1.42 }}>{item.prompt}</span>{index < aiPrompts.length ? <em style={{ color: "#bfdbfe", fontSize: "0.68rem", fontStyle: "normal", fontWeight: 950, letterSpacing: "0.1em", textTransform: "uppercase" }}>AI suggested</em> : null}</button>)}</div>
        </section>
      </div>
    </aside>
  );
